...
// Shutdown functions are delegated to /static/shutdown.js. Provide small
// shims that call the module when available so existing callers (and
// inline `onclick` handlers) continue to work during refactor.
async function shutdownServer() {
    if (typeof window.shutdownServer === 'function') return window.shutdownServer();
    // Fallback: basic confirm and client-only close
    if (!confirm('Shutdown not available. Close dashboard in this tab?')) return;
    try { if (typeof window.showShutdownSequence === 'function') window.showShutdownSequence(); else document.body.innerHTML = '<div style="padding:24px;">Dashboard closed.</div>'; } catch (e) { console.debug('fallback shutdown failed', e); }
}

function createShutdownModal(initialText) {
    if (typeof window.createShutdownModal === 'function') return window.createShutdownModal(initialText);
    return { setText: ()=>{}, remove: ()=>{} };
}

function showConfirmShutdownDialog() {
    if (typeof window.showConfirmShutdownDialog === 'function') return window.showConfirmShutdownDialog();
    return Promise.resolve(confirm('Confirm shutdown?'));
}

function showServerShutdownPage() { if (typeof window.showServerShutdownPage === 'function') return window.showServerShutdownPage(); }
function waitForServerStop(timeoutSeconds = 60, intervalMs = 1000) { if (typeof window.waitForServerStop === 'function') return window.waitForServerStop(timeoutSeconds, intervalMs); return Promise.resolve(false); }
function showServerStoppedPage(serverStopped = true) { if (typeof window.showServerStoppedPage === 'function') return window.showServerStoppedPage(serverStopped); }
function showShutdownSequence() { if (typeof window.showShutdownSequence === 'function') return window.showShutdownSequence(); }
